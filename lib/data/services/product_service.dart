...
class ProductService {
  ...
  Future<Product> createProduct(Map<String, dynamic> productData) async {
    try {
      ...
      if (response.statusCode == 201) { // 201 Created adalah response standar untuk POST
        // Asumsi Golang mengembalikan objek produk baru di 'data'
        return Product.fromJson(response.data['data']);
      }
      throw Exception('Gagal membuat produk: ${response.data['message']}');
    } on DioError catch (e) {
      throw Exception(e.response?.data['message'] ?? 'Kesalahan koneksi saat membuat produk');
    }
  }

  // Fungsi untuk menambahkan produk ke API dan daftar lokal
  Future<void> addProduct(Map<String, dynamic> data) async {
    _status = ProductStatus.loading; // Ubah status (opsional)
    notifyListeners();
  ...
  Future<void> deleteProduct(int productId) async {
    try {
      ...
      if (response.statusCode != 200) {
        ...
        final errorMessage = response.data['message'] ?? 'Gagal menghapus produk.';
  ...
  Future<Product> updateProduct(int productId, Map<String, dynamic> productData) async {
    try {
      ...
      throw Exception(e.response?.data['message'] ?? 'Kesalahan koneksi saat memperbarui produk');
    }
  }
}